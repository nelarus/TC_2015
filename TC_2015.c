...
bit TROCA_SENHA=0;
bit TRANSICAO_ETAPA=0;
bit ERRO_SENHA=0;
bit ERRO_NIVEL_DE_ACESSO=0;
bit ERRO_COMUNICACAO=0;
bit ERRO_DESCONHECIDO=0;
bit ERRO_PROTOCOLO =0;
bit ERRO_IDENTIF_CONTA=0;
			...
			if(OERR==1){
				...
				//OVERRUN
					...
					if(testar_bit(FLAGS_1,TRANSICAO_ETAPA)){
							if(dado_recebido != PROXIMA_ETAPA) {ERRO_PROTOCOLO=1;}
							...
							etapa++;}
								...
								if(dado_recebido != INICIO) {ERRO_PROTOCOLO=1;}
								...
								if(ordem==0){
							    	funcao = dado_recebido;
										if(funcao<ABERTURA_PORTA && funcao>MUDAR_SENHA) {ERRO_PROTOCOLO=1;}}
								...
								if(funcao == ABERTURA_PORTA){
											...
											if( dado_recebido != ('N'+ordem)) ERRO_PROTOCOLO=1;
											ordem++;}
															}
									...
									TRANSICAO_ETAPA=1;}
										...
										if(dado_recebido == 'A' && ordem>0) TRANSICAO_ETAPA=1;
	
										else{
											nova_senha[ordem] = dado_recebido;
											ordem++;
												if(ordem == 14) { TRANSICAO_ETAPA=1; TROCA_SENHA=1;}
															}	
					
															}
	
					else if(etapa == etapa_final){//Fim da comunicação
								if(dado_recebido != FIM) ERRO_PROTOCOLO=1;
								enviar=1;
								etapa = etapa_inicial;}
...
void main(void)
{
	...
	while(1){
		...
		if(enviar==1){
			enviar=0;
			if(	ERRO_SENHA  == 0 && ERRO_PROTOCOLO==0){
							TXREG = 'O';
							while(!TRMT){}
							TXREG = 'K';
							while(!TRMT){}}
			...
			if(ERRO_IDENTIF_CONTA){ 
							TXREG = 'E';
							while(!TRMT){}
							TXREG = 'I'; 
							while(!TRMT){} 
							ERRO_IDENTIF_CONTA=0;}
				}
